...
export function Header({ isFabricMode, setIsFabricMode }: Readonly<HeaderProps>) {
  return (
    <header className="bg-indigo-600 px-8 py-5 shadow-md">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <button
            className={`px-6 py-2.5 rounded-md font-medium transition-all ${isFabricMode ? "bg-white text-indigo-600" : "bg-indigo-500 text-white hover:bg-indigo-400"}`}
            onClick={() => setIsFabricMode(true)}
          >
            Edit
          </button>
          <button
            className={`px-6 py-2.5 rounded-md font-medium transition-all ${isFabricMode ? "bg-indigo-500 text-white hover:bg-indigo-400" : "bg-white text-indigo-600"}`}
